...
- Menu: label `บทบาทและสิทธิ์`, path `/user/rbac`, placed right after `ผู้ใช้งาน` in the
  existing subheader `ผู้ใช้งาน & สิทธิ์` (nav-config.ts).
- Module is isolated: new route `src/app/user/rbac/*` + new components; MUST NOT share with
