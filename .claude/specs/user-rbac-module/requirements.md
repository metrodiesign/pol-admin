...
Management เดิม (`src/app/user/*`, `src/components/user/*`) โดยสร้าง route ใหม่ `src/app/user/rbac/*`
...
- 1.1  THE SYSTEM SHALL แสดงรายการเมนู label `บทบาทและสิทธิ์` path `/user/rbac` ภายใต้ subheader
...
- 1.3  WHEN ผู้ใช้อยู่ที่ path ขึ้นต้นด้วย `/user/rbac` THE SYSTEM SHALL ทำให้รายการเมนู
...
**Acceptance Criteria (EARS):**
- 2.1  THE SYSTEM SHALL วาง route ของโมดูลไว้ใต้ `src/app/user/rbac/` เท่านั้น
...
- รูปแบบฟอร์ม create/edit (เต็มหน้า route `/user/rbac/new`,`/user/rbac/edit` ตามแบบ user module เดิม
