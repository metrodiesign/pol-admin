...
import type { Permission, ResourceGroup, Role } from "@/types/rbac";
import {
  grantedCount,
  groupGranted,
  groupedCatalog,
...
interface RbacRoleReadViewProps {
  role: Role;
  catalog: Permission[];
  groups: ResourceGroup[];
}

/** หน้าดูรายละเอียดบทบาทแบบเต็มหน้า (route แยก /user/rbac/read). read-only. */
export function RbacRoleReadView({
  role,
  catalog,
  groups,
}: RbacRoleReadViewProps) {
      ...
      <div className="overflow-hidden rounded-card bg-card" style={cardStyle}>
        ...
        <div className="flex flex-col gap-4 border-b border-[var(--divider)] p-6 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <Link
              href={`/user/rbac/edit?code=${encodeURIComponent(role.code)}`}
