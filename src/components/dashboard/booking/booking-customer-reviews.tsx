    ...
    <div className="dashboard-card flex flex-col h-full">
      ...
      <div className="flex-1 p-6 space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <p className="text-base font-semibold text-grey-800">{review.name}</p>
            <p className="text-base text-grey-500">{review.postedAt}</p>
          </div>
        </div>
        ...
        {/* Comment */}
        <p className="text-base text-grey-600 line-clamp-5 leading-relaxed">{review.comment}</p>
        ...
        <div className="flex flex-wrap gap-2">
          {review.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-md bg-grey-100 px-2.5 py-1 text-base font-semibold text-grey-700"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="flex gap-3 pt-2">
          <button
            type="button"
            className="flex-1 rounded-control border border-grey-300 py-2 text-base font-semibold text-grey-800 transition-colors hover:bg-grey-100"
          >
            Reject
          </button>
          <button
            type="button"
            className="flex-1 rounded-control bg-grey-800 py-2 text-base font-semibold text-white transition-colors hover:bg-grey-900"
          >
            Accept
          </button>
        </div>
