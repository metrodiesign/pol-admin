    ...
    <div className="overflow-hidden rounded-2xl border border-grey-200 bg-white flex flex-col" style={{ minWidth: 0 }}>
      ...
      <div className="flex items-center gap-2 px-3 pt-3 pb-2">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-base font-semibold text-grey-800 truncate">{booking.name}</p>
          <p className="text-base text-grey-500">{booking.bookedAt}</p>
        </div>
        <button
          type="button"
      ...
      <div className="relative mx-3 overflow-hidden rounded-xl" style={{ height: 160 }}>
        ...
        <div className="absolute bottom-2 left-2 flex items-center gap-1.5">
          {booking.isHot && (
            <span className="flex items-center gap-1 rounded-full bg-grey-800/80 px-2 py-0.5 text-base font-semibold text-white backdrop-blur-sm">
              🔥 {booking.price}
            </span>
          )}
          {!booking.isHot && (
            <span className="rounded-full bg-grey-800/80 px-2 py-0.5 text-base font-semibold text-white backdrop-blur-sm">
              {booking.price}
            </span>
          )}
        </div>
      ...
      <div className="flex items-center justify-between px-3 py-3">
        <div className="flex items-center gap-3 text-base text-grey-500">
          <span className="flex items-center gap-1">
            <Moon className="size-3.5" />
            {booking.duration}
          </span>
        </div>
        <div className="flex items-center gap-1 text-base text-grey-500">
