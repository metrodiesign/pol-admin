      ...
      <SimpleBar autoHide className="mt-4 hidden pb-1 sm:block">
        ...
        {folders.map((folder) => (
          ...
          >
            ...
            <h6 className="mt-3 text-base font-semibold leading-[22px] text-grey-800">{folder.name}</h6>
            <p className="mt-0.5 text-base text-grey-500">
            ...
            {(folder.shared.length > 0 || folder.overflow > 0) && (
              <div className="mt-3 flex items-center">
                ...
                {folder.overflow > 0 && (
                  <span className="ml-1 text-base font-medium text-grey-500">
                    +{folder.overflow}
                  </span>
                )}
              </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 sm:hidden">
        {folders.slice(0, 4).map((folder) => (
          ...
          >
            ...
            <h6 className="mt-2 text-base font-semibold leading-[22px] text-grey-800">{folder.name}</h6>
            <p className="text-base text-grey-500">{folder.size}</p>
            <p className="text-base text-grey-500">{folder.fileCount}</p>
            {folder.shared.length > 0 && (
              <div className="mt-2 flex -space-x-2">
                ...
                {folder.overflow > 0 && (
                  <span className="ml-1 text-base font-medium text-grey-500">+{folder.overflow}</span>
                )}
              </div>
            )}
          </div>
