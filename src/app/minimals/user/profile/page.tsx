      ...
      <div className="absolute bottom-0 left-0 right-0 z-10 flex h-12 items-center justify-center overflow-x-auto bg-card px-4 sm:justify-end sm:px-6">
        {TABS.map((tab) => (
          <button
            ...
            className={cn(
              "flex shrink-0 items-center gap-1.5 border-b-2 px-2 py-[9px] text-base transition-colors sm:px-3",
              activeTab === tab.key
  ...
  return (
    <div className="dashboard-card p-6">
      <textarea
        ...
        rows={4}
        className="w-full resize-none rounded-control border border-grey-200 bg-transparent px-4 py-3 text-base text-grey-800 outline-none placeholder:text-grey-400 focus:border-grey-400"
      />
      <div className="mt-6 flex items-center justify-between">
        <div className="flex gap-1">
          <button
            type="button"
            className="flex h-9 items-center gap-1.5 rounded-full bg-[rgba(145,158,171,0.16)] px-2 text-base font-semibold text-grey-800 transition-colors hover:bg-[rgba(145,158,171,0.24)]"
          >
        ...
        <button
          type="button"
          className="flex h-9 items-center rounded-control bg-grey-800 px-3 text-base font-semibold text-white transition-colors hover:bg-grey-900 dark:bg-white dark:text-grey-900 dark:hover:bg-grey-300"
        >
          Post
        </button>
      </div>
...
function AvatarStack({ likers, overflow }: { likers: string[]; overflow: number }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      {overflow > 0 && (
        <span className="text-base text-grey-500">+{overflow}</span>
      )}
    </div>
  );
}
    ...
    <div className="dashboard-card p-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-base font-semibold text-grey-800">{post.user.name}</p>
          <p className="text-base text-grey-500">{post.createdAt}</p>
        </div>
        <button
          type="button"
      ...
      {/* Content */}
      <p className="mt-6 text-base leading-[22px] text-grey-800">{post.content}</p>
            ...
            <div key={i} className="flex gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <p className="text-base font-semibold text-grey-800">{comment.user.name}</p>
                  <p className="text-base text-grey-400">{comment.createdAt}</p>
                </div>
                <p className="mt-0.5 text-base text-grey-600">{comment.content}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mt-6 flex items-center gap-3 border-t border-grey-200 pt-4">
        ...
        <div className="flex flex-1 items-center rounded-control border border-grey-200 bg-transparent px-3 py-2">
          <input
            ...
            className="flex-1 bg-transparent text-base text-grey-800 outline-none placeholder:text-grey-400"
          />
    ...
    <>
      ...
      {activeTab !== "profile" && (
        <div className="dashboard-card flex min-h-[300px] items-center justify-center p-6">
          <p className="text-base capitalize text-grey-400">{activeTab} content</p>
        </div>
      )}
    </>
  );
